...
// const privateKey = '1d1fe5bda8806ae659a52c4827219e44685ac6234222ef78571ab596e37a0527';
// const wallet = new ethers.Wallet(privateKey, provider);
// const contractAddress = '0x74C4651F8b5474063c06a1D14A2F4e202a458206';
...
//     const tx = await contract.addPatientData(patientId, encryptedData);
//     await tx.wait();
...
//     const data = await contract.getPatientData(patientId);

//     if (data.length === 0) {
//       return res.status(404).send({ error: 'No data found for this patient ID' });
//     }

//     const decryptedData = data.map(record => decryptData(record.encryptedData, encryptionKeys[patientId]));
//     res.status(200).send(decryptedData.map(record => JSON.parse(record)));
//   } catch (error) {
...
app.get('/api/patient/:id', async (req, res) => {
  try {
    ...
    for(let i=0; i<data.length; i++){
        // if(typeof data[i] ==)
        encryptedData.push(JSON.parse(data[i].encryptedData))
    }
    res.json(encryptedData);

    // const stringifiedData = JSON.parse(JSON.stringify(data, (key, value) =>
